...
>>>>>>> 732aeece7135a59aaf18f7f9791030d778446325
    ...
    foreach($images as $image)
    {
      //image name
      $imageName = $image['name']; // name provided by users
      $imageTmpName = $image['tmp_name']; // the actual file
      $imageExt = explode('.', $imageName);
      $imageRealExt = strtolower(end($imageExt));
      $newImgName = $imageExt[0].'-'.uniqid('', true).'.'.$imageRealExt;
      //upload path
      ...
      array_push($imgArray, $newImgName);
    }
    
    session_start();
    $_SESSION['imageNames'] = $imgArray;
    ...
    if($image['error'] !== 0 || empty($_POST['name']) || empty($_POST['sex']) || $_POST['litter']=='-None-' || empty($_POST['description']) || empty($_POST['price']) || (empty($_POST['availability']) && !isset($_POST['isSold']))){
      ...
      $sql = "INSERT INTO pups (name, sex, description, availability, image_name, upload_date, litter_id, isSold, buyer) VALUES ('$name', '$sex', '$description', '$availablitiy', '$newImgName', '$today', '$litter', '$isSold', '$buyer')";
